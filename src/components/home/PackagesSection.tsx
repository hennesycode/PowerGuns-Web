      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid md:grid-cols-3 gap-5 mt-14">
          {packages.map((pkg) => (
            <div
              key={pkg.id}
              className={`relative bg-[#191919] border ${
                pkg.isPopular ? "border-[#c4871a]/40 bg-[#222222]" : "border-[#c4871a]/12"
              } p-8 md:p-10 overflow-hidden transition-all duration-300 hover:border-[#c4871a]/35 hover:-translate-y-1 group`}
            >
              {pkg.isPopular && (
              ...
              <div className="font-['Rajdhani',sans-serif] font-bold text-[10px] tracking-[.35em] uppercase text-[#c4871a] mb-1.5">
                {pkg.tier}
              </div>
              ...
              <div className={`font-heading font-black text-[46px] ${pkg.isPopular ? "text-[50px]" : ""} text-[#c4871a] leading-none mb-0.5`}>
              ...
              <ul className="space-y-2.5 mb-8">
                {pkg.features.map((feat: string) => (
                  <li key={feat} className="flex items-start gap-2.5 text-[13px] text-[#c8c8c8] leading-relaxed">
                    <span className="text-[#c4871a] text-[7px] mt-[5px] flex-shrink-0">◆</span>
                    {feat}
                  </li>
                ))}
              </ul>
              ...
              <Link
                href="/reservas"
                className={`w-full block text-center py-4 font-heading font-bold text-sm tracking-[.15em] uppercase no-underline transition-all ${
                  pkg.isPopular
                    ? "bg-[#c4871a] text-[#080808] hover:bg-[#d4a244]"
                    : "bg-transparent text-[#c8c8c8] border border-[#c4871a]/35 hover:border-[#c4871a] hover:text-[#c4871a] hover:bg-[#c4871a]/7"
                }`}
              >
                RESERVAR
              </Link>
        ...
        <div className="mt-8 p-5 bg-[#c4871a]/6 border border-[#c4871a]/18 flex items-center gap-3.5">
          ...
          <p className="text-[13px] text-[#8a8a8a]">
            ¿Grupo de más de 5 personas o evento especial?{" "}
            <Link href="#contacto" className="text-[#c4871a] no-underline">
