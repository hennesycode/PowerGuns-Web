...
export async function PackagesSection() {
  ...
  try { const { packagesService } = await import("@/server/services/package.service"); const db = await packagesService.getPackages(); packages = db.map((p: Record<string, unknown>) => ({ id: p.id as number, name: p.name as string, tier: p.tier as string, tierLevel: p.tierLevel as number, price: Number(p.price), unit: (p.unit as string) || "COP por persona", features: Array.isArray(p.features) ? (p.features as string[]) : [], isPopular: Boolean(p.isPopular), orderNum: (p.orderNum as number) || 0 })); } catch {}

  return (
    <section id="paquetes" className="py-16 md:py-24 bg-[#0F0D0B]">
      <SiteShell>
        <SectionHeader eyebrow="Elige tu nivel" title={<>PAQUETES <span className="text-[#c4871a]">&amp; PLANES</span></>} description="Todos los paquetes incluyen equipo de protección, instrucción de seguridad y supervisión profesional." />
        <div className="grid md:grid-cols-3 gap-5 mt-14">
          {packages.map((pkg) => (
            <div key={pkg.id} className={`relative bg-[#171513] border ${pkg.isPopular ? "border-[#c4871a]/40 bg-[#26231F]" : "border-[#c4871a]/12"} p-8 md:p-10 overflow-hidden transition-all duration-300 hover:border-[#c4871a]/35 hover:-translate-y-1 group`}>
              {pkg.isPopular && <span className="absolute top-4 right-4 bg-[#c4871a] text-[#080706] font-['Rajdhani',sans-serif] font-bold text-[9px] tracking-[.22em] uppercase px-2.5 py-0.5 tactical-clip">MÁS POPULAR</span>}
              ...
              <div className="text-[11px] text-[#B2AAA7] mb-0.5">Desde</div>
              ...
              <div className="text-[11px] text-[#B2AAA7] mb-7">{pkg.unit}</div>
              <div className="h-px bg-[#c4871a]/18 mb-6" />
              <ul className="space-y-2.5 mb-8">
                {pkg.features.map((feat: string) => (<li key={feat} className="flex items-start gap-2.5 text-[13px] text-[#CFD1D4] leading-relaxed"><span className="text-[#c4871a] text-[7px] mt-[5px] flex-shrink-0">◆</span>{feat}</li>))}
              </ul>
              <Link href="/reservas" className={`w-full block text-center py-4 font-heading font-bold text-sm tracking-[.15em] uppercase no-underline transition-all ${pkg.isPopular ? "bg-[#c4871a] text-[#080706] hover:bg-[#d4a244]" : "bg-transparent text-[#CFD1D4] border border-[#c4871a]/35 hover:border-[#c4871a] hover:text-[#c4871a] hover:bg-[#c4871a]/7"}`}>RESERVAR</Link>
            </div>
          ))}
        </div>
