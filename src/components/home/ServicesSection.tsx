...
import { ServicesSection as ServicesSectionServer } from "@/server/services/service.service";

export async function ServicesSection() {
  const services = await ServicesSectionServer.getServices();
        ...
        <div className="grid sm:grid-cols-2 gap-0.5">
          {services.map((svc, i) => (
            <TacticalCard key={svc.id}>
              ...
              <h3 className="font-heading font-bold text-2xl uppercase tracking-[.04em] text-white mb-3">
                {svc.name}
              </h3>
