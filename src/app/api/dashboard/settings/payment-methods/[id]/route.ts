...
import { paymentMethodSchema, paymentMethodStatusSchema } from "@/lib/validations/payment-methods";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    const body = await request.json();
    const validation = paymentMethodStatusSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: validation.error.issues[0]?.message ?? "Datos inválidos" }, { status: 400 });
    }

    const method = await paymentMethodService.updateStatus(id, validation.data.isActive);
    ...
    activityService.logFromSession(session, {
      ...
      entityId: method.id,
      entityName: method.providerLabel,
      description: `Método de pago ${method.providerLabel} ${method.isActive ? "activado" : "desactivado"}`,
      status: "success",
      page: "/dashboard/configuracion",
      section: "Configuración / Métodos de pago",
      metadata: { provider: method.provider, isActive: method.isActive },
    });

    return NextResponse.json(method);
  } catch (error) {
    const session = await getSession();
    const message = error instanceof Error ? error.message : "No se pudo actualizar el estado del método de pago";
    if (session && ADMIN_ROLES.has(session.role)) {
      activityService.logFromSession(session, {
        action: "payment_method_updated",
        entityType: "payment_method",
        description: "Error al actualizar estado de método de pago",
        status: "error",
        errorMessage: message,
        page: "/dashboard/configuracion",
        section: "Configuración / Métodos de pago",
