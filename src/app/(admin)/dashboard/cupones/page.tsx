    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm">
      <div className="my-6 w-full max-w-3xl border border-[#c4871a]/15 bg-[#0F0D0B] shadow-2xl">
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2">
          <Field label="Código"><input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, "") })} className={inputClass} placeholder="POWER10" /></Field>
          <Field label="Tipo"><select value={form.discountType} onChange={(e) => setForm({ ...form, discountType: e.target.value as DiscountType, discountValue: "" })} className={inputClass}><option value="percentage">Porcentaje</option><option value="fixed">Valor monetario</option></select></Field>
          <Field label="Descuento"><div className="flex border border-[#3C3A37] bg-[#080706]"><input value={form.discountValue} onChange={(e) => setForm({ ...form, discountValue: form.discountType === "fixed" ? formatMoneyInput(e.target.value) : e.target.value.replace(/\D/g, "").slice(0, 3) })} className="min-w-0 flex-1 bg-transparent px-3 py-3 text-sm text-white outline-none" placeholder={form.discountType === "fixed" ? "50.000" : "10"} /><span className="border-l border-[#3C3A37] px-3 py-3 text-sm text-[#B2AAA7]">{form.discountType === "fixed" ? "COP" : "%"}</span></div></Field>
          <Field label="Compra mínima"><input value={form.minimumSubtotal} onChange={(e) => setForm({ ...form, minimumSubtotal: formatMoneyInput(e.target.value) })} className={inputClass} placeholder="0" /></Field>
          <Field label="Fecha inicio"><input type="date" value={form.startsAt} onChange={(e) => setForm({ ...form, startsAt: e.target.value })} className={inputClass} /></Field>
          <Field label="Fecha límite"><input type="date" value={form.expiresAt} onChange={(e) => setForm({ ...form, expiresAt: e.target.value })} className={inputClass} /></Field>
          <Field label="Usos totales"><input type="number" min={1} value={form.maxUses} onChange={(e) => setForm({ ...form, maxUses: e.target.value })} className={inputClass} placeholder="Sin límite" /></Field>
          <Field label="Usos por cliente"><input type="number" min={1} value={form.perCustomerLimit} onChange={(e) => setForm({ ...form, perCustomerLimit: e.target.value })} className={inputClass} placeholder="Sin límite" /></Field>
          <label className="flex items-center justify-between border border-[#3C3A37] bg-[#080706] px-3 py-3 sm:col-span-2"><span><span className="block text-xs uppercase tracking-[.1em] text-[#B2AAA7]">Activo</span><span className="text-xs text-[#5B5A59]">Disponible para aplicar si cumple reglas.</span></span><input type="checkbox" checked={form.isActive} onChange={(e) => setForm({ ...form, isActive: e.target.checked })} className="h-4 w-4 accent-[#c4871a]" /></label>
          <div className="sm:col-span-2">
            <Field label="Cliente único"><input value={userQuery} onChange={(e) => { setUserQuery(e.target.value); setForm({ ...form, assignedUserId: null }); }} className={inputClass} placeholder="Buscar por nombre, correo o identificación" /></Field>
