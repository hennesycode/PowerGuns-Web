...
COPY <<'EOF' start.sh
#!/bin/sh
# Limpiar registros de migraciones fallidas (no toca datos)
printf "DELETE FROM \`_prisma_migrations\` WHERE finished_at IS NULL;\n" | node node_modules/prisma/build/index.js db execute --stdin 2>/dev/null || true
# Aplicar migraciones pendientes
node node_modules/prisma/build/index.js migrate deploy 2>&1 || echo "[OK] Migraciones ya aplicadas o DB necesita atencion manual"
exec node server.js
EOF
